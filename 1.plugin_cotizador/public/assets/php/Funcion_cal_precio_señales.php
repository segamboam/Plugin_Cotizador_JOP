...
function precios_señales($categoria,$area,$url){
    ...
    $valores =array_column($Matriz_filtrada, 4);
    $busca = $area;
    if (in_array($area, $valores)) {
        $id=array_search($busca,$valores);
        $precio=array_column($Matriz_filtrada,5)[$id];
        ...
        $area_fil=$mayor;
        $pmayor=array_filter($Matriz_filtrada, function($var) use ($area_fil) {return $var[4]==$area_fil;});
        $pmayor=array_column($pmayor,5)[0];
        $area_fil=$menor;
        $pmenor=array_filter($Matriz_filtrada,function($var) use ($area_fil) {return $var[4]==$area_fil;});
        $pmenor=array_column($pmenor,5)[0];
